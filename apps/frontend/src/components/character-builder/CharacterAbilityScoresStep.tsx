    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {ABILITY_SCORES.map((ability) => {
          ...
          return (
            <div key={ability.key} className="p-4 rounded-lg bg-gray-50">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-medium text-gray-900">{ability.name}</h3>
                  <p className="text-sm text-gray-500">{ability.abbr}</p>
                </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <input
                  ...
                  min="3"
                  max="20"
                  className="flex-1 px-3 py-2 text-center border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  onChange={(e) =>
                    handleScoreChange(
                      ability.key,
        ...
        <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-3">
          {ABILITY_SCORES.map((ability) => (
            <div key={ability.key} className="flex justify-between">
              <span className="text-blue-700">{ability.abbr}:</span>
