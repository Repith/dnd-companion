...
- **sessionId** – optional UUID of the session context (null for campaign-level or global events).
- **campaignId** – optional UUID of the campaign context.
- **global** – optional boolean indicating if the event is globally visible (default false).

#### Event Scoping Levels

Events are categorized by their scope to enable appropriate filtering and distribution:

- **Session-level events**: Have `sessionId` set, `campaignId` optional. These are specific to a particular game session and include combat actions, dice rolls, and session-specific updates.
- **Campaign-level events**: Have `campaignId` set, `sessionId` null. These affect the entire campaign and include quest updates, character level-ups, and campaign-wide notifications.
- **Global events**: Have `global` set to true, `campaignId` optional. These are system-wide events like announcements, patches, or cross-campaign achievements.

#### Event Handlers

Event handlers are functions that process specific event types and perform side effects. They implement the `EventHandler<T>` interface:

```typescript
type EventHandler<T extends GameEvent> = (event: T) => Promise<void> | void;
```

**Handler Responsibilities:**

- **State Updates**: Modify character stats, inventory, quest status, etc.
- **Notifications**: Send real-time updates to connected clients via SSE
- **Logging**: Record events for audit and replay purposes
- **Side Effects**: Trigger additional events, update caches, or perform cleanup
- **Validation**: Ensure event data consistency and business rule compliance

#### Event Flow Architecture

```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Event Source  │───▶│   EventBus      │───▶│ Event Handlers  │
│ (Services)      │    │ (Central Hub)   │    │ (Subscribers)   │
└─────────────────┘    └─────────────────┘    └─────────────────┘
                              │
                              ▼
                       ┌─────────────────┐
...
The EventBus serves as the central communication mechanism, ensuring all events flow through a single, consistent pipeline. Services publish events to the bus, which then distributes them to registered handlers based on event type and filters.
